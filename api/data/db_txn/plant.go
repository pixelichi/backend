...
func AddPlantOrAbort(c *gin.Context, db *db.Store, createPlantParam *db.CreatePlantParams) (*db.Plant, error) {
	plant, err := db.CreatePlant(c, *createPlantParam)
	...
	if err != nil {
		if pqErr, ok := err.(*pq.Error); ok {
			...
			case "unique_violation":
				err := server_error.NewForbiddenError("Plant already exists.")
